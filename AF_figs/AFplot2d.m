...
function [h_fig, slp, int, r2, nonan] =  AFplot2d(ps_string,ps_col,datetime_num_ps,data_corr_ps,cs_string,cs_col,datetime_num_cs,data_corr_cs,...
...
if strcmp('\circ',label_string)
    line (datetime_num_ps(index_ps(1):index_ps(2)),data_corr_ps(index_ps(1):index_ps(2),ps_col),'linestyle','none','color','k','linewidth',1,'marker','o','markersize',4,'markerfacecolor','k');
    line (datetime_num_cs(index_cs(1):index_cs(2)),data_corr_cs(index_cs(1):index_cs(2),cs_col),'linestyle','none','color','b','linewidth',1,'marker','+','markersize',4,'markerfacecolor','b');
else
    line (datetime_num_ps(index_ps(1):index_ps(2)),data_corr_ps(index_ps(1):index_ps(2),ps_col),'linestyle','-','color','k');
    line (datetime_num_cs(index_cs(1):index_cs(2)),data_corr_cs(index_cs(1):index_cs(2),cs_col),'linestyle','--','color','b');
end;
...
% legend
if ~strcmp('\circ',label_string)
    h_leg = legend (ps_string,cs_string,'location','Best'); set (h_leg,'fontsize',10);legend (h_leg,'boxoff');
else
    h_leg = legend (ps_string,cs_string,'location','Best'); set (h_leg,'fontsize',10);legend (h_leg,'boxon');
end;
...
ylabel ([ps_string ' - ' cs_string],'fontsize',10);
...
corr_r      =   corrcoef(data_scat_ps(index_nonnan),data_scat_cs(index_nonnan));
...
text (xpos(1)+0.05*xrng, ypos(1)+0.9*yrng,['mean = ',num2str(nanmean(dif),'%4.2f')],'fontsize',10);
text (xpos(1)+0.05*xrng, ypos(1)+0.8*yrng,['median = ',num2str(nanmedian(dif),'%4.2f')],'fontsize',10);
text (xpos(1)+0.05*xrng, ypos(1)+0.7*yrng,['std = ',num2str(nanstd(dif),'%4.2f')],'fontsize',10);

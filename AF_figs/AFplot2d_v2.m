...
function [h_fig, slp, int, r2, nonan] =  AFplot2d_v2(ps_string,ps_col,datetime_num_ps,data_corr_ps,unca_corr_ps,cs_string,cs_col,datetime_num_cs,data_corr_cs,...
...
if strcmp('\circ',label_string)
    ...
    if sum(~isnan(unca_corr_ps(index_ps(1):index_ps(2),ps_col)))~=0         % add random uncertainty to graph if data avail. 
        ...
        for i = 1:size(brack_idx,1)
            he = jbfill(datetime_num_ps(brack_idx(i,1):brack_idx(i,2))', (data_corr_ps(brack_idx(i,1):brack_idx(i,2),ps_col)+unca_corr_ps(brack_idx(i,1):brack_idx(i,2),ps_col))', (data_corr_ps(brack_idx(i,1):brack_idx(i,2),ps_col)-unca_corr_ps(brack_idx(i,1):brack_idx(i,2),ps_col))', rgb('light grey'),rgb('grey'), 1,1);
        end
    end
...
% legend
if strcmp('\circ',label_string)
    h_leg = legend ([ha hb], ps_string,cs_string,'location','Best'); set (h_leg,'fontsize',10);legend (h_leg,'boxoff');
else
    if sum(~isnan(unca_corr_ps(index_ps(1):index_ps(2),ps_col)))~=0
        h_leg = legend ([ha he hb], ps_string,'random unc.',cs_string,'location','Best'); set (h_leg,'fontsize',10);legend (h_leg,'boxon');
    else
        h_leg = legend ([ha hb], ps_string,cs_string,'location','Best'); set (h_leg,'fontsize',10);legend (h_leg,'boxon');
    end
end;
...
xlabel (['Date [' timetick_mode ']'],'fontsize',10);

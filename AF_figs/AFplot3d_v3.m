function [h_fig, slp, int, r2, nonan] =  AFplot3d_v3(...
...
% this version is for cases were 2 datasets are compared to 1 (3D)
% v2 - added orthogonal regression (08/2017)
% v3 - update histogram plotting to use Freedman-Diaconis rule (04/2019)
...
[~,~,beta1] = rTLS(data_scat_mn(index_nonnan1),data_scat_cs1(index_nonnan1));
...
% Calculate linear regression using Total Least Squares / Orthogonal (TLS)
[~,~,beta2] = rTLS(data_scat_mn(index_nonnan2),data_scat_cs2(index_nonnan2));
...
[~,bin_edge]=histcounts([dif1; dif2],'binmethod','fd');

bin_width = mean(diff(bin_edge));

bin_ctrs = bin_edge(1:end-1)+bin_width/2;

[N1,~] = histcounts(dif1,bin_edge);
[N2,~] = histcounts(dif2,bin_edge);

bar(bin_ctrs,N1',1,'FaceColor','none','EdgeColor','b')
hold on
bar(bin_ctrs,N2',1,'FaceColor','none','EdgeColor','r','LineStyle','-.')
hold off

% h1 = bar(bin_ctrs-bin_width*0.25,N1',0.5,'b');
% hold on
% h2 = bar(bin_ctrs+bin_width*0.25,N2',0.5,'r');
% hold off

if ~(nansum(dif1 == 0) && nansum(dif2 == 0))
